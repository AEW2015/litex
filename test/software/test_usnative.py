...
class TestUSNativeFirmware(unittest.TestCase):
    ...
    def profile(self, rate=2400, extra="", check=""):
        clock, cl, cwl, rd, wr = (300000000,17,12,2,3) if rate == 2400 else (333333333,19,14,0,1)
        return f'''
...
{extra}
#include "profile.h"
static int selected_rd=-1, selected_wr=-1;
static void command_p{rd}(unsigned value) {{ (void)value; selected_rd={rd}; }}
static void command_p{wr}(unsigned value) {{ (void)value; selected_wr={wr}; }}
int main(void) {{
 USNATIVE_RD_COMMAND(1); USNATIVE_WR_COMMAND(1);
 {check}
 return selected_rd!={rd} || selected_wr!={wr};
}}
'''
...
""")
    ...
    def test_ddr_rate_does_not_overflow_signed_int(self):
        ...
        function = source[start:source.index("\n}\n", start) + 3]
        for clock in (300000000, 333333333):
